...
export default {
  data() {
    return {
      ...
      services: [],
      tempActiveServices: [],
      tempDistance: this.$route.params.distance
        ? this.$route.params.distance
        : 20,
  ...
  methods: {
    ...
    fillFromSuggestion(event) {
      ...
      let selectedResult = this.searchResults.find(
        (result) => result.address.freeformAddress === event.target.innerHTML
      );
    ...
    handleQuery() {
      ...
      axios
        .get(
          "https://api.tomtom.com/search/2/search/" + this.searchTerm + ".json",
          {
            params: {
  ...
  <div class="card" data-bs-theme="light">
    <div class="card-body">
      ...
      <div class="d-flex justify-content-center flex-wrap w-100">
        <div
          class="d-flex flex-column suggestions-container align-items-center w-50"
        >
          <span>Where</span>
          <input
            @change="handleQuery()"
            type="text"
            class="input-styler big-input w-100 my-3"
            id="where"
          />
          <ul class="suggestions p-0 d-none" id="sg-list">
            <li
              v-for="(suggestion, index) in this.searchResults"
              :key="index"
              @click="fillFromSuggestion($event)"
            >
        ...
        <div class="d-flex flex-column align-items-center" v-if="isSearchRoute">
          ...
          <input
            ...
            type="number"
            id="rooms"
            class="small-input text-dark my-3 input-styler"
          />
        </div>
        <div class="d-flex flex-column align-items-center" v-if="isSearchRoute">
          <span>Beds</span>
        ...
        <router-link
          ...
          :to="{
            ...
            params: {
              ...
              activeServices: tempActiveServices.join('-'),
            },
          }"
